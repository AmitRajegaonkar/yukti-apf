...
DIRECT_US_ACCESS_TOKEN = os.getenv("DIRECT_US_ACCESS_TOKEN")
ALERT_FACES_FOLDER_ID = os.getenv("ALERT_FACES_FOLDER_ID")
CAMERA_0_LOCATION = os.getenv("camera_0_location", "Unknown Location")

direct_us_url = "http://localhost:8055"
...
def upload_frame_to_directus(image_path):
    ...
    try:
        with open(image_path, 'rb') as f:
            s = requests.Session()
            headers = {"Authorization": f"Bearer {DIRECT_US_ACCESS_TOKEN}"}
            response = s.post(
                f"{direct_us_url}/files",
                headers=headers,
                files={"file": ("image.jpg", f, "image/jpeg")},
                data={"folder": "a418f3a5-0f13-478f-b61f-92434b3218f9"}  # Replace with your folder ID
            ).json()
            if "data" in response:
                ...
                if conf > 0.70:
                    ...
                    if class_name.lower() in ['rifle', 'hand gun', 'knife', 'weapon', 'gun']:
                        ...
                        if weapon_detected:
                            ...
                            if file_id:
                                send_directus_alert(location=CAMERA_0_LOCATION, file_id=file_id)
