                ...
                try:
                    if flag:
                        ...
                        with open("frame.jpg",'rb') as f:
                            ...
                            print(response['id'])
                            # Get camera location from env, fallback to camera ID
                            camera_loc = os.getenv(f"CAMERA_{camera}_LOCATION") or os.getenv(f"camera_{camera}_location") or str(camera)
                            
                            to_add = s.post("http://localhost:8055/items/alert_db",headers=headers,json={"frame":response['id'],"camera_location":camera_loc,"aadhaar_found":name}).json()
                            print(to_add)
...
def get_criminal_encodings(exited:ValueProxy,criminals_db:DictProxy):
    ...
    while not exited.get():
        ...
        try:
            s = requests.Session()
            response = s.get("http://localhost:8055/items/criminal_db",headers=headers).json()
            if 'data' not in response:
                print(f"Error fetching criminal_db: {response}")
                time.sleep(2)
                continue
            criminals_aadhaar_full:list = response['data']
            ...
            for criminal in criminals:
                resp = s.get(f'http://localhost:8055/items/aadhaar_db/{criminal}',headers=headers).json()
                # print(f"Aadhaar DB Response for {criminal}:", resp)
                try:
                    if 'data' in resp and 'face_embeddings' in resp['data']:
                         # Check if face_embeddings has 'data' key or is the list itself
                        embeddings_data = resp['data']['face_embeddings']
                        if isinstance(embeddings_data, dict) and 'data' in embeddings_data:
                            encodings_full = embeddings_data['data']
                        elif isinstance(embeddings_data, list):
                            encodings_full = embeddings_data
                        else:
                            print(f"Unexpected format for {criminal}: {type(embeddings_data)}")
                            continue
                        encodings.append(encodings_full)
                    else:
                        print(f"Missing data for {criminal}: {resp.keys()}")
                except Exception as inner_e:
                    print(f"Error processing {criminal}: {inner_e}")
